...
{
	this.$ = {};
	this.$.contenedor  	    = $target;
	this.$.carousel 		= this.$.contenedor.find('.carousel');
	this.$.prev 	 		= this.$.contenedor.find('.prev');
	this.$.next 		    = this.$.contenedor.find('.next');
	this.$.play 		    = this.$.contenedor.find('.play');
	this.$.slides_contenedor = this.$.contenedor.find('.slides .items');
	this.$.slides 			= this.$.slides_contenedor.find('.item');
	this.$.seek_bar			= this.$.contenedor.find('.seek-bar');
	this.$.progress_bar		= this.$.contenedor.find('.progress-bar');

	this.$.choice			= this.$.contenedor.find('.choice');
	this.$.tracks 			= this.$.choice.find('.tracks');
	this.$.track 			= this.$.tracks.find('.track');
	this.$.speakers			= this.$.track.find('.speakers');
	this.$.music			= this.$.contenedor.find('.music');
...
{	
	if (currentIndex != index) { //Avoid to start over the audio by clicking on the current music and mute the audio
		...
		this.$.carousel.css({
			"background-image": "url(https://universegakku.demo.site/wp-content/uploads/2018/03/" + index + ".jpg)"
		});
		...
		this.$.music[0].setAttribute('src', 'https://github.com/Alexis25u/music/raw/master/' + index + '.mp3');
		this.changeMusic();
...
var $carousel = new Carousel( $('.contenedor ') );
